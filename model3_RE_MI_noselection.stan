...
transformed parameters{
  ...
  real <lower = 0> sigma[K]; // variance, for now assume the outcome are uncorrelated
...
model {
  ...
  logtau ~ normal(0,sigma_prior_logtau);
...
//  real u_rep[N];
//  for(i in 1:N){
//    u_rep[i] = normal_rng(0,sigma_alpha);
//    for(k in 1:K){
//      y_rep[i,k] = normal_rng(intercept[k] + u_rep[i] + x_mat[i,k,k]*beta[k]+ pscore[i]*gamma[k], sigma[k]);
//    }
